...
from src import DIM_TO_FORMAT, SIM_METHODS, FORMATS, load_yaml
...
@click.option('-d', '--dimensions',          type=str, help="dimensions of the simulation, format: N[xN[xN[...]]")
@click.option('-m', '--method',              type=click.Choice(SIM_METHODS), help="simulation method")
@click.option('-r', '--rule',                type=int, help="rule to simulate")
@click.option('-i', '--iterations',          type=int, help="iterations to simulate")
@click.option('-n', '--neighborhood-radius', type=int, help="neighborhood radius to use")
@click.option('-o', '--out',                 type=click.Path(dir_okay=False, writable=True), help="path to save the output to")
@click.option('--initial-state',             type=int, help="initial simulation state")
@click.option('--parallelize',               is_flag=True, default=None, help="enabled parallel calculation of cells per state transition")
@click.option('--scaling',                   type=int, help="scaling to apply to output")
@click.option('--format',                    type=click.Choice(FORMATS), help="format to output as, this skips the default which is to simply use the most suitable for the given number of dimensions")
def cli(dimensions, method, rule, iterations, neighborhood_radius, initial_state, parallelize, scaling, out, format=None, **draw_options):
	"""
	...
	if sim_method is None:
		raise click.BadParameter(f'unkown simulation method, options are {", ".join(SIM_METHODS)}')
	...
	if format is None:
		if len(dimensions) + 1 > len(DIM_TO_FORMAT):
			raise click.BadParameter(f'can only render simulations with up to {len(DIM_TO_FORMAT) - 1} dimensions')
		else:
			draw_method = DIM_TO_FORMAT[len(dimensions)]
	else:
		if format not in FORMATS:
			raise click.BadParameter(f'unkown output format, options are {", ".join(FORMATS)}')
		else:
